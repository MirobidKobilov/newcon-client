                                    ...
                                    {items.map((item) => (
                                        <div key={item.id} className="bg-white border border-slate-200 rounded-xl p-5 hover:shadow-md transition-shadow">
                                            ...
                                            {item.permissions && item.permissions.length > 0 && (
                                                <div className="mt-2">
                                                    <div className="text-xs text-slate-400 font-medium uppercase mb-1">Разрешения</div>
                                                    <div className="flex flex-wrap gap-1">
                                                        {item.permissions.map((p) => (
                                                            <span key={p.id} className="inline-block px-2 py-1 text-xs rounded bg-gray-100 text-gray-700">{p.name}</span>
                                                        ))}
                                                    </div>
                                                </div>
                                            )}
